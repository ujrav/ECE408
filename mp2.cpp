...
__global__ void matrixMultiply(float * A, float * B, float * C,
    ...
    for (i = 0; i < numAColumns/TILE_WIDTH + offset; i++){
        rowA = row;
        colA = tx + i*TILE_WIDTH;
        rowB = ty + i*TILE_WIDTH;
        colB = col;
        ...
        if (rowA >= numARows || colA >= numAColumns){
            ...
            subTileA[tx][ty] = A[rowA*numAColumns + colA];    
        }
        ...
        if (rowB >= numBRows || colB >= numBColumns){
            ...
            subTileB[tx][ty] = B[rowB*numBColumns + colB];
        ...
        for (j = 0; j < TILE_WIDTH; j++){
            cVal += subTileA[j][ty] * subTileB[tx][j];
        }
        __syncthreads();
    }
...
int main(int argc, char ** argv) {
    ...
    wbLog(TRACE, "The dimensions of B are ", numCRows, " x ", numCColumns);
    ...
    wbLog(TRACE, "grid dimensions x = ", DimGrid.x, " y = ", DimGrid.y);
